...
export const createBanner = async (req, res) => {
  try {
    ...
    if (!title || !type || !image) {
      return res.status(400).json({ success: false, message: 'Title, type, and image are required.' });
    }
    ...
    res.status(400).json({ success: false, message: 'Upload error.', error: error.message });
  }
};
